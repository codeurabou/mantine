...
## Example to using Group for Responsive

```tsx
import {Group} from '@matine/core'
...
function Landin () {
      return(
          <Group
             ...
             sx={(theme) => ({ backgroundColor: theme.colors.blue[1] })}
             spacing={220}
            p="md"
          >
            <div>like right content</div>
            <div>like left content</div>
          </Group>
      )
}
```
